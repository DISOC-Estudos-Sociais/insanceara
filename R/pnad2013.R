pnad2013 <- function(){
  cat("=== INICIANDO PROCESSAMENTO PNAD 2013 ===\n")
  dados_dom <- read_pnada(microdata = "data/raw_data/DOM2013.txt", input_txt = "data/raw_data/input DOM2013.txt") |> 
    pnada_labeller(dictionary.file = "data/raw_data/Dicionário de variáveis de domicílios - PNAD 2013.xls")
  
  dados <- read_pnada(microdata = "data/raw_data/PES2013.txt", input_txt = "data/raw_data/input PES2013.txt") |> 
    pnada_labeller(dictionary.file = "data/raw_data/Dicionário de variáveis de pessoas - PNAD 2013.xls") |> 
  ...
  dados <- dados |>
    dplyr::select(UF, V4602,
                  ...
                  # segurança alimentar
                  V4623A) |>
    dplyr::mutate(ano="PNAD 2013", .before = 1) |> 
    dplyr::rename(uf = UF,
                  estrato = V4602,
                  situacao = V4105,
                  peso = V4611,
    ...
    dplyr::mutate(situacao = forcats::fct_collapse(situacao,
                  ...
                  san = forcats::fct_collapse(san,
                                              "Segurança alimentar"=c("Não tem morador menor de 18 anos e segurança alimentar", "Tem morador menor de 18 anos e segurança alimentar"),
  ...
  readr::write_rds(dados, "data/clean_data/pnad2013.RDS")
